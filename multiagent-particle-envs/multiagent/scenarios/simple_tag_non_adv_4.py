...
class Scenario(BaseScenario):
    ...
    def reset_world(self, world):
        ...
        for i, target in enumerate(world.targets):
            if not target.boundary:
                target.state.p_pos = np.random.uniform(-0.8, +0.8, world.dim_p)
                '''
                if bool_test:
                    if i ==0:
                ...
                if agent.target_id == entity.entity_id:
                    ...
                    if tmp < dist_min + threshhold:
                        ...
                        if train_or_test is True:
                            entity.state.p_pos = np.random.uniform(-0.8, +0.8, world.dim_p)
    ...
    def observation(self, agent, world):
        ...
        for entity in world.targets:
            if not entity.boundary and agent.target_id == entity.entity_id:
                entity_pos.append(entity.state.p_pos - agent.state.p_pos)
        # communication of all other agents
        comm = []
        other_pos = []
