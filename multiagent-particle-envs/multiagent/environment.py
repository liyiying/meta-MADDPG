...
class MultiAgentEnv(gym.Env):
    ...
    def step(self, action_n):
        ...
        info_n = {'n': []}
        self.agents = self.world.policy_agents
        # take a temp memory for storing the position of last time
        tmp = []
        for agent in self.agents:
            tmp_agent1 = copy.deepcopy(agent)
            tmp.append(tmp_agent1.state.p_pos)
        ...
        for i, agent in enumerate(self.world.agents):
            #delta_pos = agent.state.p_pos - tmp[i].state.p_pos
            delta_pos = np.sqrt(np.sum(np.square(agent.state.p_pos - tmp[i])))
            agent.accu_dis += delta_pos
